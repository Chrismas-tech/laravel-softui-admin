<div>
    ...
    <div class="d-flex justify-content-between align-items-center p-3">
        <a href="{{ route('admin.youtube-videos.create') }}"
            ...
            <span>Add a new Album Photo</span>
        </a>
    ...
    <table class="table align-items-center mb-0 table-striped">
        <thead>
            <tr>
                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Album Photo</th>
                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Actions</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($collectionPagination as $album)
                <tr>
                    <td width="50%;">
                        <div class="d-flex justify-content-center">
                            <span class="badge badge-pill bg-gradient-primary">#{{ $album->id }}
                                {{ $album->name }}
                            </span>
                        </div>
                        <livewire:album-photos.upload-photos :index="$album->id"/>
                    </td>
                    <td width="50%;">
                        <div class="d-flex justify-content-center align-items-center">
                            <div class="form-check form-switch d-flex justify-content-center align-items-center me-3">
                                <input class="form-check-input" wire:click="toggleSelection({{ $album->id }})"
                                    type="checkbox" {{ in_array($album->id, $selected) ? 'checked' : '' }}
                                    id="flexSwitchCheckDefault">
                            </div>
                            <button wire:click="editEntryModal({{ $album->id }})" type="button"
                                class="btn bg-gradient-info me-3 mb-0 mt-3 mb-3">Edit</button>
                            <button wire:click="duplicateEntryModal({{ $album->id }})" type="button"
        ...
        <x-slot name="content">
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('videoName') is-invalid @enderror"
                    wire:model="videoName" value="{{ $modelName }}">
                @error('videoName')
            ...
            <div class="mb-3">
                ...
                <textarea rows="5" class="form-control @error('videoIframe') is-invalid @enderror" wire:model="videoIframe">{!! $modelIframe !!}</textarea>
                @error('videoIframe')
                    <span class="text-danger text-sm">{{ $message }}</span>
                @enderror
            </div>
